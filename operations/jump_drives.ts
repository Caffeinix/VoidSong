...
import { JumpDrive, Ship } from '../model/ship.js';
...
export class JumpDrives {
  public static clone(jumpDrive: DeepReadonlyObject<JumpDrive>): JumpDrive {
  ...
  public static update(
    ...
    const jumpDrive: DeepReadonlyObject<JumpDrive> = ship.systems.jumpDrive;
    ...
    if (jumpDrive.charging && jumpDrive.destination) {
      const distance = Positions.getDistanceInWorldUnits(
        ship.position, jumpDrive.destination);
  ...
  private static _calculateActualDestination(ship: DeepReadonlyObject<Ship>): Position {
    const jumpDrive: DeepReadonlyObject<JumpDrive> = ship.systems.jumpDrive;
    ...
    const intendedDistance =
      Positions.getDistanceInWorldUnits(ship.position, jumpDrive.destination);
    ...
    const actualVector = new WorldPoint(
      originalVector.x + displacementVector.x, originalVector.y + displacementVector.y);
